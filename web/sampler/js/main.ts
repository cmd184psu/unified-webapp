...
const { THEMES, ThemeManager, HamburgerMenu, openModal, confirmDialog, alertDialog, promptDialog, showToast } = shared;
...
// The Hamburger section (phase2 §5 Step 4.5). One HamburgerMenu mounted in
// the header, registering EVERY item kind once, because B4.2's sampler row is
// the gallery that exercises them: two actions, one link, a separator, a
// section header, a `render` slot, a `when()`-guarded item, and the standard
// themePicker bound to the same `themes` instance the header <select> and the
// Theme picker section already use — three views of one state.
//
// The demo rows below mirror buildModalDemos()'s row shape (a button that runs
// the demo and a <pre> carrying the source that produced it), as
// buildToastDemos() does, and they exist because three of the class's
// properties are invisible from the drawer alone: that open()/toggle() are
// callable from outside, that `when()` is re-read on the NEXT open rather than
// cached, and that the slot's <select> is still the same node after a
// close/open cycle.
function buildMenu(): void {
  ...
  // Read by the guard on every open, never captured by value — flipping this
  // has to change the drawer's contents without any addItem/removeItem call.
